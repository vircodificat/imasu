...
pub fn decode_instruction(bits: u32) !Instruction {
    ...
    switch (opcode) {
        0b00000 => { // lb, lh, lw, ld, lbu, lhu, lwu
            const imm = decode_i_type_immediate(bits);
        ...
        0b00101 => { // auipc
            const imm = decode_u_type_immediate(bits);
        ...
        0b01000 => { // sb, sh, sw, sd
            const imm = decode_s_type_immediate(bits);
        ...
        0b11000 => { // B-type intructions: beq, bne, blt, bge, bltu, bgeu
            const imm = decode_b_type_immediate(bits);
        ...
        0b11011 => { // jal
            const imm = decode_j_type_immediate(bits);
            return Instruction{ .J = .{ .opcode = .jal, .rd = rd, .imm = imm } };
        },
        0b11100 => {
            if (funct3 != 0b000) { // csr opcodes
                const csrno: u12 = @bitCast(decode_i_type_immediate(bits));
