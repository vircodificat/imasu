// RISC-V Instruction opcodes an operands
...
pub const Instruction = union(enum) {
    ...
    // and a 20-bit signed immediate value
    U: struct {
        opcode: U,
        rd: u5,
        imm: i20,
...
test "enumerated all instructions" {
    ...
    try testing.expect(num_variants(R) == 10 + 5 + 8 + 5);
    // lr sc + 9 operations for word and double-word size
    try testing.expect(num_variants(AMO) == 2 * 11);
    // 15 RV32I + 6 RV64 only
    try testing.expect(num_variants(I) == 15 + 6);
    // write, set bits, clear bits, with register and immediate
    try testing.expect(num_variants(CSR) == 6);
    // stores for powers of 2 bytes up to 8
    try testing.expect(num_variants(S) == 3 + 1);
    // 6 branch predicates
    try testing.expect(num_variants(B) == 6);
    // lui and auipc
    try testing.expect(num_variants(U) == 2);
    // jal only
    try testing.expect(num_variants(J) == 1);
    try testing.expect(num_variants(Special) == 8);
}
